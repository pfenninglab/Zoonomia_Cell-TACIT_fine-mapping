...
#SBATCH --time 0-5
#SBATCH --dependency=afterok:829435
...
CODEDIR=${SETDIR}/code/raw_code/SCREEN_cCREs; DATADIR=${SETDIR}/data/raw_data/SCREEN_cCREs
...
PREFIX=$(stat -c "%n" $DATADIR/annotation/*.${POP}.1.annot.gz | sed 's!.*/!!' | cut -d'.' -f1| sort | uniq )
CELLTYPES=$(stat -c "%n" $DATADIR/annotation/*.${POP}.1.annot.gz | sed 's!.*/!!' | cut -d'.' -f2| sort | uniq )

for CELL in $CELLTYPES; do
	###########################################################
	## run LD score regression to get estimate SNP heritability 
	FOREGROUND=$(stat -c "%n" $DATADIR/annotation/*${CELL}*${POP}.1.annot.gz | sed "s/${POP}.*/${POP}.,/g" | uniq | sed -z "s/\n//g"  )
	...
	--ref-ld-chr ${FOREGROUND}${GWASDIR}/1000G_ALL_Phase3_hg38_files/baselineLD_v2.2/baselineLD_v2.2.${POP}. \
	--frqfile-chr ${GWASDIR}/1000G_ALL_Phase3_hg38_files/plink/1000G.${POP}.HM3. \
	--out ${DATADIR}/enrichments/${CELL}.${GWAS_Label}.${POP}

	####################################################################################
	## format logs file to make .results file, --print-snps doesn't make a .results file
	log2results ${DATADIR}/enrichments/${CELL}.${GWAS_Label}.${POP}.log
done
