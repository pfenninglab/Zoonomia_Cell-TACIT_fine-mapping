...
def parseArgument():
	...
	parser.add_argument("--outputFileName", required=True,\
			help='bed file where the selected summits will be written')
...
def convertChromNames(options):
	...
	bedFile = None
	if options.gzip:
		# The bed file is gzipped
		bedFile = gzip.open(options.bedFileName)
	else:
		# The bed file is not gzipped
		bedFile = open(options.bedFileName)
	outputFile = None
	if options.gzip:
		# The output file should be gzipped
		outputFile = gzip.open(options.outputFileName, 'wb')
	else:
		# The output file should not be gzipped
	...
	for line in bedFile:
		...
		if chrom not in chromNameDict:
			...
			if options.includeMissingChrom:
				# Include the region as chrNone
				outputFile.write("chrNone" + "\t" + "\t".join(lineElements[1:len(lineElements)]) + "\n")
				continue
			print("Problem: Chromosome " + chrom + " not in list of chromosomes")
			continue
		outputFile.write(chromNameDict[chrom] + "\t" + "\t".join(lineElements[1:len(lineElements)]) + "\n")
	bedFile.close()
	outputFile.close()
