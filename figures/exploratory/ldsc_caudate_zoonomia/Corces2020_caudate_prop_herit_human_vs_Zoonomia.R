...
enrich_fn =here(PROJDIR,'prop_herit') %>% 
...
save_fn = here(SETDIR,'rdas','caudate_conservation_prop_herit_Zoonomia.rds')
saveRDS(enrichments, file = save_fn)

#################################
## make plots for presentation ##
dir.create(here(SETDIR,'plots'))
dir.create(here(SETDIR,'plots','prop_herit_plots'))
...
plot_fn = here(SETDIR,'plots',paste('zoonomia_caudate', 
...
for(lab in plot_traits){
  ...
  plot_fn1 = 
    here(SETDIR,'plots', 'prop_herit_plots',
                       paste('zoonomia_caudate',lab, cell,
