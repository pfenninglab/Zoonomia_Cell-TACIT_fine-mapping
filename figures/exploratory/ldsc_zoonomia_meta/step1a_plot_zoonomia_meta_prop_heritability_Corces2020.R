...
enrichments = input %>% 
  mutate(
    ...
    group_meta = Categories %>% ss('allPeaks.',2) %>% 
...
save_fn = here(PLOTDIR,'rdas','zoonomia_meta_prop_heritability_Corces2020.rds')
saveRDS(enrichments, file = save_fn)

#################################
## make plots for presentation ##
dir.create(here(PLOTDIR,'plots'))
dir.create(here(PLOTDIR,'plots','prop_herit_Corces2020'))
