...
BGNAME=BulkATACseq.All_Roadmap_cCREs_phyloConsAccl_phastCons.BG
BGFILE=${DATADIR}/peaks/${BGNAME}.bed.gz

# cat ${BG1} ${BG2} ${PHYLOCONS} ${PHYLOACCL} ${PHASTCONS} \
# 	${DATADIR}/peaks/BulkATACseq*Primates#0.mappable.bed.gz | \
...
if [[ $HASFILE == "FALSE" ]]; then 
	sbatch --mem 45G -p pool1,pfen1,pfen3 --time 12:00:00 --job-name=${BGNAME}.AFR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
	-i ${BGFILE} -n ${BGNAME} -g hg38 -p AFR -o $ANNOTDIR
fi

FILE=${ANNOTDIR}/${BGNAME}.EUR.@.l2.M; checkFile
if [[ $HASFILE == "FALSE" ]]; then 
	sbatch --mem 45G -p pool1,pfen1,pfen3 --time 12:00:00 --job-name=${BGNAME}.EUR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
	-i ${BGFILE} -n ${BGNAME} -g hg38 -p EUR -o $ANNOTDIR
fi

CELLTYPES="M1ctx Liver"
