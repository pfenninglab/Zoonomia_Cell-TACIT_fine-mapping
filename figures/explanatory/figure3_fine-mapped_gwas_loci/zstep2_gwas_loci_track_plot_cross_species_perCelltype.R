...
if(FALSE){
  fn= list.files(path = 'data/raw_data/ldsc_zoonomia_meta/peaks',
                 pattern = 'Corces2020.', full.names = T) %>%
    grep(pattern = 'allPeaks', value = TRUE)
  outfn = gsub("data/raw_data/ldsc_zoonomia_meta/peaks",
               'figures/explanatory/figure3_fine-mapped_gwas_loci/bed', fn)
  outfn= gsub('.bed.gz', '.bed', outfn)
  thecall = paste('zcat', fn, '> ', outfn)
  parallel::mclapply(thecall, system, mc.cores = 10)
  
  fn2= list.files(path = 'data/raw_data/ldsc_caudate_zoonomia/CellTACIT',full.names = T)
  outfn2 = gsub("data/raw_data/ldsc_caudate_zoonomia/CellTACIT",
               'figures/explanatory/figure3_fine-mapped_gwas_loci/CellTACIT', fn2)
  outfn2= gsub('.bed.gz', '.bed', outfn2)
  thecall = paste('zcat', fn2, '> ', outfn2)
  parallel::mclapply(thecall, system, mc.cores = 10)
}
...
loci$score[as.numeric(names(inds))] = sapply(inds, function(ind){
  finemap_df2[ind,] %>% mutate(tmp = PIP * CellTACIT_score * numTrait) %>% pull(tmp) %>% max()
})
loci = loci[loci$score > 50]
...
for(idx in seq_along(loci)) {
  ...
  ##########################
  ## 3) construct tracks  ##
  pdf('tmp2.pdf', height = 3*1, width = 4.75*1, onefile = F)
  trackList <- trackList(  zooMeta_tracks, CellTACIT_track, finemap_track, 
                           loci_track, zoo_track, genetic_map_track, genes, 
                          heightDist = c(4, 2.5, 1.5, 2, 1, .5, 1))
  names(trackList)[which(names(trackList)== 'CellTACIT_track')]= 'Cell-TACIT Score'
  ...
  for (i in names(zooMeta_tracks)) setTrackYaxisParam(trackList[[i]],  "draw", FALSE)
  for (i in names(col_celltypes)) { 
  ...
  for(i in c(names(zoo_fn), 'Recombination Map')) {
    ...
    setTrackYaxisParam(trackList[[i]], "gp", list(lwd = .5, cex = .4)) 
  }
  ...
  setTrackYaxisParam(trackList[['Polyfun SNPs']], "gp",gparList)
  setTrackStyleParam(trackList[['Polyfun SNPs']], "ylabgp",gparList)
  ...
  pdf(plot_fn, height = 3*1.5, width = 4.75*1.5, onefile = F)
