...
#SBATCH --job-name=vBiasAway
#SBATCH --gres=gpu:1
#SBATCH --mem=63G
#SBATCH --array=1-40
#SBATCH --error=logs/cnn_ocp_b_biasAway10x_%A_%a.txt
#SBATCH --output=logs/cnn_ocp_b_biasAway10x_%A_%a.txt
...
# #############################################
# ## train CNN models with default parameters across species
...
python train_singleTask_CNN_classifier_OCP.py --mode 'train' --out_dir $DATADIR \
	...
	--train_fasta_neg $DATADIR/fasta/hg38_${CELLTYPE}_fold${FOLD}_train_biasAway10x.fa.gz \
	--valid_fasta_pos $DATADIR/fasta/hg38_${CELLTYPE}_fold${FOLD}_valid_positive.fa.gz \
	--valid_fasta_neg $DATADIR/fasta/hg38_${CELLTYPE}_fold${FOLD}_valid_biasAway10x.fa.gz \
	--prefix $PREFIX
