...
enrichments_meta = enrichments %>%
  ...
  lapply(function(df){
    ...
    df$Enrichment_SE = df$Observed_scale_h2_SE / df$h2 / df$Proportion_of_SNPs
    ...
    ret = df %>% select(Clade:Categories) %>% 
      summarise_if(is.numeric, mean) %>%
      summarise_if(is.character, ~ paste(sort(unique(.x)), sep = ';')) %>%
      bind_cols(tab1)
    return(ret)
  }) %>% rbindlist()


enrichments_meta = enrichments %>% filter(!is.na(Enrichment)) %>%
  mutate(
    Enrichment_SE = Observed_scale_h2_SE / h2 / Proportion_of_SNPs) %>% 
  group_by(plotGroup, celltype, trait, group, Order) %>%
  nest() %>% mutate(
    # meta-analyze the enrichments
    tmp = meta.summaries(Enrichment, Enrichment_SE, method="random"),
    # Enrichment = tmp$summary, 
    # Enrichment_SE = tmp$se.summary,
    # meta-analyze the coefficients
    tmp2 = meta.summaries(Coefficients, Coefficient_SE, method="random"),
    # Coefficients = tmp2$summary, 
    # Coefficient_SE = tmp2$se.summary,
    # meta-analyze the observed h2
    tmp3 = meta.summaries(Observed_scale_h2, Observed_scale_h2_SE, method="random"),
    # Observed_scale_h2 = tmp3$summary, 
    # Observed_scale_h2_SE = tmp3$se.summary
    ) %>% unnest() %>% select(-c(tmp, tmp2, tmp3))
