...
plot_fn = here(PLOTDIR, 'plots', 'fine-mapped_SNP_ecdfByGroup_quartiles_main.pdf')
pdf(plot_fn, height = 2.5, width = 5)
ggplot(snps_annotated_df%>% filter(celltype %in% celltypes[c(1:3,6)], 
                                   quantile %in% c('quartile4', 'non-overlap')), 
                                   aes(x = PIP, color = group, linetype = quantile)) + 
  ...
  scale_color_manual(values = group_col[c(1:6, 8)]) + 
