...
function usage()
{
    ...
    echo "-s SPECIES    sourceSpecies in the Cactus file e.g. {Homo_sapiens, Mus_musculus, Macaca_mulatta}"
...
if [[ ! -f ${HALLIFTEDSFILE}.gz || $OVERWRITE == 'TRUE' ]]
    ...
    then halLiftover $TMPDIR/$(basename $CACTUSFILE) $SOURCE $SUMMITFILE $TARGET $HALLIFTEDSFILE
else
...
SIMPLEBED=${TMPDIR}/${NAME}.6col.tmp.bed; cut -f1-6 $UNIQUEBED > $SIMPLEBED
if [[ ! -f ${HALLIFTEDTFILE}.gz || $OVERWRITE == 'TRUE' ]]
    # hal-liftover the peaks de novo
    then halLiftover $TMPDIR/$(basename $CACTUSFILE) $SOURCE $SIMPLEBED $TARGET $HALLIFTEDTFILE
else
