...
#SBATCH --mem=20G
#SBATCH --error=logs/est_herit_predActive_NeuronSubtype_%A_%a.txt
#SBATCH --output=logs/est_herit_predActive_NeuronSubtype_%A_%a.txt
...
OUTDIR=${DATADIR}/prop_herit_NeuronSubtype; mkdir -p $OUTDIR
cd $CODEDIR; 
...
OUT=${OUTDIR}/zoonomia_meta.NeuronSubtype.${GWAS_Label}.${POP}.${CELL}.${LAB}
