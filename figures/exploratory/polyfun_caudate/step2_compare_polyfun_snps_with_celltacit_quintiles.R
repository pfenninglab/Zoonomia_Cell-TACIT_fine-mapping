...
##########################################
## 2) read in the Cell TACIT Age quintiles
peak_fn = list.files('data/raw_data/ldsc_celltacit_age_decile/peaks', 
                     pattern = '.bed.gz', full.names = T) %>% str_subset('quintile')
...
overlapMat = lapply(peak_list, countOverlaps, query = snps_gr) %>% 
  ...
  mutate(celltype = ss(peak, '\\.', 1), quintile = ss(peak, '\\.', 2)) %>%
  filter(value ==1) %>%
  pivot_wider(id_cols = c('name'), names_from = celltype, 
              values_from = quintile, values_fill = 'non-overlap')
...
quintiles = c('0-20%', '20-40%', '40-60%', '60-80%' , '80-100%')
names(quintiles) = paste0('quintile', 1:5); 
...
snp_enrichment_df = lapply(groups, function(y){
  ...
  mutate(group = factor(group), 
         quantile = factor(quantile), quantile = recode(quantile, !!!quintiles))

dir.create( here(PLOTDIR, 'tables'), showWarnings = F)
save_fn = here(PLOTDIR, 'tables', 'fine-mapped_SNP_enrichmentByGroup_quintiles_logistic_regression.xlsx')
snp_enrichment_df %>% writexl::write_xlsx(save_fn)


################################
## 4) plot the log odds ratio
plot_fn = here(PLOTDIR, 'plots', 'fine-mapped_SNP_logEnrichmentByGroup_quintiles_main.pdf')
...
plot_fn2 = here(PLOTDIR, 'plots', 'fine-mapped_SNP_logEnrichmentByGroup_quintiles_other.pdf')
...
plot_fn = here(PLOTDIR, 'plots', 'fine-mapped_SNP_enrichmentByGroup_quintiles_main.pdf')
...
plot_fn2 = here(PLOTDIR, 'plots', 'fine-mapped_SNP_enrichmentByGroup_quintiles_other.pdf')
...
save_fn = here(PLOTDIR, 'tables', 'fine-mapped_SNP_enrichmentByTrait_quintiles_logistic_regression.xlsx')
snp_enrichment_df2 %>% writexl::write_xlsx(save_fn)


################################
## 7) plot the log odds ratio
plot_fn = here(PLOTDIR, 'plots', 'fine-mapped_SNP_enrichmentByTrait_quintiles_neurons.pdf')
pdf(plot_fn, height = 2.5, width = 5)
...
plot_fn2 = here(PLOTDIR, 'plots', 'fine-mapped_SNP_enrichmentByTrait_quintiles_glia.pdf')
pdf(plot_fn2, height = 2.5, width = 5)
