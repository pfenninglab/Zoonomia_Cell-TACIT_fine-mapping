...
enrichments_fn = here(PLOTDIR,'rdas','zoonomia_meta_prop_heritability_NeuronSubtype.rds')
enrichments = readRDS(file = enrichments_fn)

zoo_meta_lm = enrichments %>%
  group_by(trait, celltype) %>% nest() %>%
  mutate(
    fit = map(data, ~ lm(Enrichment ~ Time.Since.Split.from.Human.TimeTree.median + 
                           peaktype, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  ...
  mutate(term = gsub('Time.Since.Split.from.Human.TimeTree.median', 'hMYA', term),
         term = gsub('peaktypepredActive', 'predActive', term)) %>%
  pivot_wider(values_from = c(estimate, std.error, statistic, p.value), 
              names_from = term)
  
zoo_meta_lm = zoo_meta_lm %>%
  arrange(sign(`estimate_predActive`) * log10(p.value_predActive),
  sign(estimate_hMYA) * log10(p.value_hMYA))
          
dir.create(here(PLOTDIR,'tables'), showWarnings = F)
lm_tsv_fn = here(PLOTDIR,'tables','zoonomia_meta_prop_heritability_NeuronSubtype_linRegCoefficients.xlsx')
