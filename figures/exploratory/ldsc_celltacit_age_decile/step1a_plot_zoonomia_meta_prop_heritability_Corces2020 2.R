...
LABEL='ldsc_celltacit_age_decile'
PLOTDIR=here('figures/exploratory/', 'ldsc_celltacit_age_decile')
...
##############################################
# 1) read in the Zoonomia tree and quantile list
...
enrich_fn =here(DATADIR,'prop_herit_Corces2020') %>% 
...
dir.create(here(PLOTDIR,'rdas'), recursive = T, showWarnings = F)
save_fn = here(PLOTDIR,'rdas','celltacitAge_quantile_prop_heritability_Corces2020.rds')
saveRDS(enrichments, file = save_fn)

#################################
## make plots for presentation ##
dir.create(here(PLOTDIR,'plots','prop_herit_Corces2020'), recursive = T, showWarnings = F)
...
###############
#### make plots
for(peak in unique(enrichments$peaktype)){
  for(cell in unique(enrichments$celltype)){
    plot_fn = here(PLOTDIR,'plots',paste('cellTACITage', peak, cell, 
                                         'prop_herit_enrichments.ppt.pdf', sep = '_'))
    pdf(width = width_ppt, height = height_ppt, file = plot_fn, onefile = T)
    for(lab in plot_traits){
      ## subset data to cell type and GWAS
      tmp = enrichments %>% filter(celltype %in% cell, trait %in% lab, peaktype %in% peak)
      if(nrow(tmp) > 0){
        ## the SNP enrichments ##
        pp1 = ggplot(data = tmp, aes(x = quantile, y = Enrichment)) +
          ...
          geom_jitter(aes(fill = quantile, shape = peaktype), position=position_dodge(width=2)) +
          scale_shape_manual(values = c(21:23)) + 
          scale_linetype_manual(name = "Zoonomia", values = c(1, 1), 
                                guide = guide_legend(title.position="top", nrow =1,override.aes = 
                                                       list(size = 1,color = c("#882255", "#888888")))) + 
          theme_bw(base_size = 10) + ggtitle(paste(cell, lab, sep = ', ')) +
          xlab(paste0('CellTACIT Age Quantile')) + ylab(paste0('Heritability Enrichment')) + 
          guides(fill = guide_legend(nrow =1, title.position="top", override.aes = list(pch = 21 ,size = 4)),
                 shape = guide_legend(nrow =1, title.position="top", override.aes = list(size = 4))) + 
        ...
        pp2 = ggplot(data = tmp, aes(x = quantile, y = Proportion_of_h2g)) +
          # add phyloP enrichments lines
          ...
          scale_linetype_manual(name = "Zoonomia", values = c(1, 1), 
                                guide = guide_legend(title.position="top", nrow =1,override.aes = 
                                                       list(size = 1,color = c("#882255", "#888888")))) + 
          theme_bw(base_size = 10) + ggtitle(paste(cell, lab, sep = ', ')) +
          xlab(paste0('CellTACIT Age Quantile')) + ylab(paste0('Proportion SNP Heritability')) + 
          guides(fill = guide_legend(nrow =1, title.position="top", override.aes = list(pch = 21 ,size = 4)),
                 shape = guide_legend(nrow =1, title.position="top", override.aes = list(size = 4))) + 
          theme(legend.position = "bottom", legend.text=element_text(size=font_fig),
                legend.title=element_text(size=font_fig), legend.key.height=unit(.5,"line"))
        ...
        plot_fn1 = here(PLOTDIR,'plots', 'prop_herit_Corces2020', 
                        paste('celltacitAge', peak, lab, cell, 'prop_herit_enrichments.ppt.pdf', sep = '_'))
