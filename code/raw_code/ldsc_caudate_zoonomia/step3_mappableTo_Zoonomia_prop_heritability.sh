...
#SBATCH --job-name=zoo_h2
#SBATCH --ntasks-per-node=1
#SBATCH --dependency=afterany:1436732
#SBATCH --cpus-per-task=1
#SBATCH --mem=10G
#SBATCH --error=logs/zoonmia_heritability_%A_%a.txt
#SBATCH --output=logs/zoonmia_heritability_%A_%a.txt
#SBATCH --array=1-64
...
OUTDIR=${DATADIR}/prop_herit; mkdir -p $DATADIR $OUTDIR

echo "Working on ${GWAS_Label}."
...
CTS_FN=${SETDIR}/data/tidy_data/Zoonomia_data/caudate_zoonomia_mapped.${CELL}.${POP}.hg38.celltypes.ldcts
...
OUT=${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}.${LAB}
...
if [[ ! -f ${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}.agg.gz ]]; then
gunzip ${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}*.results.gz
awk ' FNR == 2 || NR==1 {print}' \
	${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}*.results |
	gzip >  ${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}.agg.gz
gzip ${OUTDIR}/caudate_zoonomia_mapped.${GWAS_Label}.${POP}.${CELL}*.results
fi
done
