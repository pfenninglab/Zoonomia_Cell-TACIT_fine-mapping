...
log2results() {
	IN=$1
	OUT=$(echo $IN| sed 's/.log$/.results.gz/g')
...
for PEAKTYPE in $PEAKTYPES; do
	## run LD score regression to get estimate SNP heritability for the full-length peaks, remove All b/c collinearity
	CELLTYPES=$(stat -c "%n" $DATADIR/annotation/${PEAKTYPE}.*.full.*${POP}.1.annot.gz | sed "s/${POP}.*/${POP}.,/g"| sed '/All/d' | sort| uniq | sed -z 's/\n//g')
	ldsc.py --h2 $GWAS --print-coefficients --print-snps ${SNPLIST} --n-blocks 1000 \
	...
	--out ${DATADIR}/enrichments/${PEAKTYPE}.full.${GWAS_Label}.${POP}
	## format logs file to make .results file, --print-snps doesn't make a .results file
	log2results ${DATADIR}/enrichments/${PEAKTYPE}.binary.full.${GWAS_Label}.${POP}.log

	## run LD score regression to get estimate SNP heritability for the core peaks, remove All b/c collinearity
	CELLTYPES=$(stat -c "%n" $DATADIR/annotation/${PEAKTYPE}.*.core.*${POP}.1.annot.gz | sed "s/${POP}.*/${POP}.,/g"| sed '/All/d' | sort| uniq | sed -z 's/\n//g')
	ldsc.py --h2 $GWAS --print-coefficients --print-snps ${SNPLIST} --n-blocks 1000 \
	--w-ld-chr ${GWASDIR}/1000G_ALL_Phase3_hg38_files/weights/1000G.${POP}.weights.hm3_noMHC. \
	--ref-ld-chr ${CELLTYPES}${GWASDIR}/1000G_ALL_Phase3_hg38_files/baselineLD_v2.2/baselineLD_v2.2.${POP}. \
	--frqfile-chr ${GWASDIR}/1000G_ALL_Phase3_hg38_files/plink/1000G.${POP}.HM3. \
	--out ${DATADIR}/enrichments/${PEAKTYPE}.core.${GWAS_Label}.${POP}
	## format logs file to make .results file, --print-snps doesn't make a .results file
	log2results ${DATADIR}/enrichments/${PEAKTYPE}.binary.core.${GWAS_Label}.${POP}.log
done
