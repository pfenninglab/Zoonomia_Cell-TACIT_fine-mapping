...
python ${CODEDIR}/train_singleTask_CNN_classifier_OCP.py --mode 'predict' --out_dir $DATADIR2 \
--model_name $MODEL --predict_out "effectSNP_${CELLTYPE}" --predict_fasta $EFFECT_FASTA2

# noneffect allele in celltypes
python ${CODEDIR}/train_singleTask_CNN_classifier_OCP.py --mode 'predict' --out_dir $DATADIR2 \
--model_name $MODEL --predict_out "nonEffSNP_${CELLTYPE}" --predict_fasta $NONEFF_FASTA2
done
