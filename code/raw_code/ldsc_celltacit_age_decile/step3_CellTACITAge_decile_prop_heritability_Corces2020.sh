...
#SBATCH --job-name=decile
#SBATCH --dependency=afterany:1690917
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=10G
#SBATCH --error=logs/est_herit_decile_%A_%a.txt
#SBATCH --output=logs/est_herit_decile_%A_%a.txt
...
CTS_FN=${DATADIR}/CellTACITage_decile.${CELL}.${POP}.hg38.celltypes.ldcts
...
OUT=${OUTDIR}/CellTACITage_decile.Corces2020.${GWAS_Label}.${POP}.${CELL}.${LAB}
...
OUTFILE=${OUTDIR}/CellTACITage_decile.Corces2020.${GWAS_Label}.${POP}.${CELL}.agg.gz
# if [[ ! -f $OUTFILE || "$OUTFILE" -ot ${CELL2}.1.l2.M ]]; then
gunzip ${OUTDIR}/CellTACITage_decile.Corces2020.${GWAS_Label}.${POP}.${CELL}*.results.gz
awk ' FNR == 2 || NR==1 {print}' \
	${OUTDIR}/CellTACITage_decile.Corces2020.${GWAS_Label}.${POP}.${CELL}*.results |
	gzip > ${OUTFILE}
gzip ${OUTDIR}/CellTACITage_decile.Corces2020.${GWAS_Label}.${POP}.${CELL}*.results
# fi
done
