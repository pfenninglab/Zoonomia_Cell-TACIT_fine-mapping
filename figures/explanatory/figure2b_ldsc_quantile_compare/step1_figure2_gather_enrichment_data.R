...
keepPeakTypes =  c('Human' = 'Hg peak', 'Macaque' = 'Rm -> Hg', 'Mouse' = 'Mm -> Hg')
keepPeakTypes2 = setNames(names(keepPeakTypes), keepPeakTypes)
...
## CellTACIT Score, Caudate snATAC-seq cell types
...
enrichments_meta = here('figures/exploratory/', 'ldsc_zoonomia_meta',
  ...
  readRDS() %>% mutate(datatype = 'CellTACIT Score') %>%
...
## CellTACIT Score, bulk M1 and liver ATAC-seq
...
enrichments_quantile = here('figures/exploratory/', 'ldsc_celltacit_age_decile',
                                   'rdas','celltacitAge_quantile_prop_heritability_Corces2020.rds') %>%
  readRDS() %>% mutate(datatype = 'CellTACIT Age') %>%
  filter(trait %in% keepTraits, quantile %in% keepPeakTypesQuantile ) %>%
  mutate(celltype2 = as.character(celltype),
         peaktype = keepPeakTypesQuantile2[quantile])

# table(enrichments_quantile$peaktype)


#############################################
## 2) Combine dataframes and reset x labels
peaktype2 = c(keepPeakTypesPhyloP2, keepPeakTypes2, keepSpecies2, 
              keepGroupMeta2, keepPeakTypesQuantile2) %>% unique()
datatype2 = c('PhyloP', 'ATAC-seq', 'Mappable to', 'CellTACIT Score', 'CellTACIT Age')
...
enrichments = list(enrichments_phyloP, enrichments_orthologs,  enrichments_mappable, 
                   enrichments_meta, enrichments_quantile) %>% 
  rbindlist(fill=TRUE) %>% 
