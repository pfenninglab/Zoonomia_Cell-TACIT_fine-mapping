...
function usage()
{
    echo "halper_map_narrowPeaks.sh takes in a narrowpeakfile in one genome coordinate"
    echo "and uses halLiftover and HALPER to map to the orthologous regions in a target genome."
    echo "Summits of narrowpeak files will be used to anchor peaks."
    echo "Example run call:"
    echo ""
    echo "sbatch halper_map_narrowPeaks.sh -s sourceSpecies -t targetSpecies -o /dir/to/output/ -p myPeaks.narrowPeak "
    echo ""
    echo ""
