...
pos_fasta_fn = file.path(PROJDIR, 'fasta', paste(genome, names(human_enhList), 'positive.fa', sep = '_'))
posFasta = mapply(writeGRangesToFasta, gr = human_enhList, file = pos_fasta_fn, genome = genome)
...
for(fold in names(folds)){
  pos_fasta_fn = file.path(PROJDIR, 'fasta', paste(genome, cell, fold, split, 'positive.fa', sep = '_'))
