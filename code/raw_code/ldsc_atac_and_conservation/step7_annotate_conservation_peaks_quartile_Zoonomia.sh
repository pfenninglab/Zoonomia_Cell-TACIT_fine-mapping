...
mkdir -p $OUTDIR/peaks_quartile $ANNOTDIR
...
CTS_AFR_FN=${OUTDIR}/conservation_peaks_quartile.${PEAKTYPE}.AFR.hg38.celltypes.ldcts; > $CTS_AFR_FN
CTS_EUR_FN=${OUTDIR}/conservation_peaks_quartile.${PEAKTYPE}.EUR.hg38.celltypes.ldcts; > $CTS_EUR_FN

############################################
## annotate cell types mapped across species
for BED in ${OUTDIR}/peaks_quartile/Corces2020.*.${PEAKTYPE}.*.bed.gz; do
...
sbatch --mem 10G -p short1,pool1,interactive,pool3-bigmem,pfen1 --time=2:00:00 --array=1-22 \
...
if [[ $HASFILE == "FALSE" ]]; then 
sbatch --mem 10G -p short1,gpu,interactive,pool1,pool3-bigmem,pfen1 --time=2:00:00 --array=1-22 \
