...
def evaluate_SHAP_scores(args, fg, fg_ids, bg):
    ...
    fg_list = np.array_split(fg, fg.shape[0] // 100)
    ...
    for ind in range(0,len(fg_list)):
        print(f'Computing SHAP values: {(ind + 1) * 100} of {fg.shape[0]}.')
...
def main(args):
    ...
    if args.mode == 'evaluate':
        ...
        imp_fn = f'{args.out_dir}/shap/{args.predict_out}.pos{args.numFg}.imp_SHAP_scores.txt.gz'
        ...
        hyp_fn = f'{args.out_dir}/shap/{args.predict_out}.pos{args.numFg}.hyp_SHAP_scores.txt.gz'
        ...
        imp_out = pd.DataFrame({'id': df.index.str.split(pat = '_').str[1], 
            'y_pred_logit': df['y_pred_logit'],
            'SHAP_score': [print_2darray(imp[i,:]) for i in range(imp.shape[0])]})
        imp_fn = f'{args.out_dir}/shap/{args.predict_out}.predict.imp_SHAP_scores.txt.gz'
        print(f'Writing importance scores to {imp_fn}.')
        imp_out.to_csv(imp_fn, index = False, sep = '\t', header = False)
        #
        hyp_out = pd.DataFrame({'id': df.index.str.split(pat = '_').str[1], 
            'y_pred_logit': df['y_pred_logit'],
            'SHAP_score': [print_2darray(hyp[i,:])  for i in range(hyp.shape[0])]})
        hyp_fn = f'{args.out_dir}/shap/{args.predict_out}.predict.hyp_SHAP_scores.txt.gz'
        print(f'Writing hypothetical importance scores to {hyp_fn}.')
        hyp_out.to_csv(hyp_fn, index = False, sep = '\t', header = False)
        #
        fasta_out = onehot2fasta(fg,ids,args)
...
if __name__ == '__main__':  
    ...
    #
    parser.add_argument("--numFg", type=str, default = 2000, help="number of positives to evaluate with SHAP scores. Note that all predict fasta are scored.", required=False)
    parser.add_argument("--numBg", type=str, default = 500, help="number of negatives in background to compute SHAP scores", required=False)
    parser.add_argument("--seq_length", type=str, default = 501, help="number of negatives in background to compute SHAP scores", required=False)
