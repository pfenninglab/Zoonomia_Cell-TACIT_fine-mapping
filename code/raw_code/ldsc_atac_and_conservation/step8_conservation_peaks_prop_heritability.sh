...
OUTDIR=${DATADIR}/prop_herit_quartile; mkdir -p $OUTDIR
cd $CODEDIR; 
...
CTS_FN=${DATADIR}/conservation_peaks_quartile.${PEAKTYPE}.${POP}.hg38.celltypes.ldcts
...
OUT=${OUTDIR}/conservation_peaks_quartile.Corces2020.${GWAS_Label}.${POP}.${LAB}
...
OUTFILE=${OUTDIR}/conservation_peaks_quartile.${PEAKTYPE}.Corces2020.${GWAS_Label}.${PEAKTYPE}.${POP}.agg.gz
if [[ ! -f $OUTFILE || \
	$(zcat ${OUTFILE}) == '' || \
	"$OUTFILE" -ot ${CELL2}.1.l2.M ]]; then
gunzip ${OUTDIR}/conservation_peaks_quartile.Corces2020.${GWAS_Label}.${POP}*${PEAKTYPE}*.results.gz
awk ' FNR == 2 || NR==1 {print}' \
${OUTDIR}/conservation_peaks_quartile.Corces2020.${GWAS_Label}.${POP}*${PEAKTYPE}*.results |
gzip > ${OUTFILE}
gzip ${OUTDIR}/conservation_peaks_quartile.Corces2020.${GWAS_Label}.${POP}*${PEAKTYPE}*.results
fi
