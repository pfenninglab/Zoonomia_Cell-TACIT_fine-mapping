...
CTS_AFR_FN=${DATADIR}/CellTACITage_quartile.${CELL}.AFR.hg38.celltypes.ldcts; > $CTS_AFR_FN
CTS_EUR_FN=${DATADIR}/CellTACITage_quartile.${CELL}.EUR.hg38.celltypes.ldcts; > $CTS_EUR_FN
## Get the annotations from the zoonomia species
for BED in ${DATADIR}/peaks/*$CELL*quartile?.bed.gz; do
...
sbatch --mem 4G -p short1,interactive,pool1,pool3-bigmem --array=1-22 \
