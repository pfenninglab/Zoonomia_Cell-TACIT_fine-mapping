...
enrich_fn =here(DATADIR,'prop_herit_NeuronSubtype') %>% 
...
enrichments = input %>% 
  mutate(
    ...
    match = ss(file, '\\.', 3), 
    celltype = ss(file, '\\.', 5) %>% factor(c('PV', 'SST', "VIP"))) %>% 
...
save_fn = here(PLOTDIR,'rdas','zoonomia_meta_prop_heritability_NeuronSubtype.rds')
saveRDS(enrichments, file = save_fn)

#################################
## make plots for presentation ##
dir.create(here(PLOTDIR,'plots'), showWarnings = F)
dir.create(here(PLOTDIR,'plots','prop_herit_NeuronSubtype'), showWarnings = F)
