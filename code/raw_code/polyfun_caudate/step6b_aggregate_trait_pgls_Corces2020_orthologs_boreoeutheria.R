...
i_am(file.path(CODEDIR, 'step6b_aggregate_trait_pgls_Corces2020_orthologs_boreoeutheria.R'))
...
peaksMappable = readRDS(outPeaks_numMappable) %>% rename('name' = 'peakNames')
...
pgls_df = pgls_fn %>% lapply(fread) %>% rbindlist(idcol = 'file') %>%
  mutate(celltype = case_when(grepl('oligo',file) ~ 'Oligo', TRUE ~ ''),
         zooTrait = ss(file, '_', 2), 
         speciesSubset = case_when(grepl('boreo', file) ~ 'Boreoeutheria', 
                                   TRUE ~ 'Euarchontoglires')) %>%
  rename('Enhancer' = 'peakNames', 'Exp_Pvalue' = 'PGLS_Pvalue') %>%
  ...
  group_by(zooTrait, speciesSubset, celltype)  %>% dplyr::select(-file) %>%
  mutate(PGLS_FDR = p.adjust(PGLS_Pvalue, 'fdr')) %>%
  ungroup() %>% arrange(PGLS_FDR) %>% 
  relocate(c(PGLS_FDR), .after = PGLS_Pvalue)

sum(pgls_df$PGLS_FDR < 0.05) # 4
sum(pgls_df$PGLS_FDR < 0.10) # 11

## annotate peaks w/ nearby gene
pgls_gr = pgls_df %>% mutate(value = gsub('^hg38:|:250$','',peakNames), name = peakNames) %>%
  dplyr::select(c(name, value)) %>% deframe() %>% GRanges()
...
pgls_xlsx = here(DATADIR, 'tables', 'trait_pgls_Corces2020_finemapped_snps_brainResid.xlsx')
...
pgls_df %>% top_n(sum(pgls_df$PGLS_FDR < 0.1), -PGLS_FDR) %>% data.frame() %>%
  dplyr::select(c(label, Gene.Symbol, annotation, PGLS_FDR,
