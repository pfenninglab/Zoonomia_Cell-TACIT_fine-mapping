...
# 29.539    80.213    10.459   105.042   115.761    15.759    24.157    13.067
...
#  Astro INT_Pvalb Microglia    MSN_D1    MSN_D2    MSN_SN     Oligo       OPC 
# 53.679    34.756    37.982    51.163    50.196    44.991    33.155    45.820 

## ratio of positives to non-enhancer orthologs
lengths(mouse_enhList) / lengths(nonEnh_peakList)
#     Astro INT_Pvalb Microglia    MSN_D1    MSN_D2    MSN_SN     Oligo       OPC 
# 0.5502897 2.3078893 0.2753673 2.0530852 2.3061798 0.3502701 0.7286081 0.2851811 


# export postive sequences to summit-centered 501bp fasta file
positiveSet = summitCenter(mouse_enhList, width = 501)
positiveSplit_list = lapply(positiveSet, splitPeakSet, testSet = testSet, 
                            validSet = validSet, useCol = 'col')
sapply(positiveSplit_list, lengths)

# export postive sequences to summit-centered 501bp fasta file
negativeSet = summitCenter(nonEnh_peakList, width = 501)
negativeSplit_list = lapply(negativeSet, splitPeakSet, testSet = testSet, 
                            validSet = validSet, useCol = 'col')
sapply(negativeSplit_list, lengths)
...
save_fn = file.path(PROJDIR, 'rdas', paste('cnn_enhancer_non-enhancer_split', genome,'.rda', sep = '_'))
