...
ARCHDIR=file.path(PROJDIR,'ArchR_BICCN_CP_4D')

### find the pre-processed arrow files
if(FALSE){
  ArrowFiles = list.files(path = file.path(PROJDIR,'arrow'),
                          pattern = '4D.arrow', full.names = TRUE)
  print(ArrowFiles)

    ### make an ArchR Projects ######
  proj = ArchRProject( ArrowFiles = ArrowFiles, outputDirectory = ARCHDIR,
...
proj <- addIterativeLSI(
  ...
  clusterParams = list( # See Seurat::FindClusters
    resolution = c(.1, .4, .8, 1, 1), # lower this if noticing subtle batch effects
...
# add imputation
proj <- addImputeWeights(proj, dimsToUse = 1:30, reducedDims = "Harmony")


# add umap
proj <- addUMAP( ArchRProj = proj, reducedDims = "Harmony", scaleDims = FALSE,
                 name = "UMAP", nNeighbors = 50, minDist = .5, dimsToUse = 1:30,
                 metric = "cosine", force = TRUE)
proj = saveArchRProject(ArchRProj = proj)

# add clusters
proj <- addClusters(input = proj, reducedDims = "Harmony", 
                    method = "Seurat", name = "Clusters", dimsToUse = 1:30,
