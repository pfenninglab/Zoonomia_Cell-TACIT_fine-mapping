...
BGNAME=Corces2020_caudate.All_Roadmap_DHS_cCREs.BG
BGFILE=${DATADIR}/${BGNAME}.bed.gz

# cat ${BG1} ${BG2} ${BG3} ${BG4} | zcat | cut -f 1-3 | \
...
## for background
# sbatch --mem 5G -p pfen1 --job-name=${BGNAME}.AFR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
# -i ${BGFILE} -n ${BGNAME} -g hg38 -p AFR -o $ANNOTDIR
# sbatch --mem 5G -p pfen1 --job-name=${BGNAME}.EUR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
...
for BED in ${DATADIR}/peak/*.narrowPeak.gz; do
NAME=$(basename $BED | sed 's/.narrowPeak.gz//g')
if [[ ! -f "${ANNOTDIR}/${NAME}.AFR.1.l2.ldscore.gz" ]]; then 
echo "Annotations for ${NAME} not found."
sbatch --mem 5G -p pfen1 --job-name=${NAME}.AFR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
-i ${BED} -n ${NAME} -g hg38 -p AFR -o $ANNOTDIR
fi
if [[ ! -f "${ANNOTDIR}/${NAME}.EUR.1.l2.ldscore.gz" ]]; then 
echo "Annotations for ${NAME} not found."
sbatch --mem 5G -p pfen1 --job-name=${NAME}.EUR ${GWASDIR}/scripts/annotate_bed_LDSC_1000G_hg19_hg38.sh \
