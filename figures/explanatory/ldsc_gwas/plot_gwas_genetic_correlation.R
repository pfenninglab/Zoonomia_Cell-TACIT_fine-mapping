...
dir.create('figures/explanatory/ldsc_gwas')
dir.create('figures/explanatory/ldsc_gwas/plots')
...
rg_plot_fn = here(PROJDIR, 'plots/ldsc_rg_gwas_correlation.pdf')
pdf(rg_plot_fn, height = 7, width = 8)
pp = Heatmap(corMat, na_col = "gray", cluster_rows = clust,
        cluster_columns = clust, name = 'GWAS Rg',
        ...
        layer_fun = function(j, i, x, y, width, height, fill) {
          v = pindex(corMat, i, j)
          if(sum(v > .1)/length(v) > 0.7) {
            grid.rect(gp = gpar(lwd = 2, fill = "transparent"))
          }}
        )
ha = HeatmapAnnotation(
  Group = pheno$group[match(pheno$trait, rownames(corMat))], 
  Size = pheno$signif_group[match(pheno$trait, rownames(corMat))], 
  col = list(Group = group_col, Size = c(Larger_GWAS = 'black', Smaller_GWAS ='white')))
